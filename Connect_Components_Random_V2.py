...
def get_data(G_bike, name):
    ...
    for it in range(to_iterate):  # loop over N-1 components
        ...
        if closest_ij2['i'] != closest_ij2['j']:  # Sanity check, the nodes have to be different
            ...
            delta2.append(p_delta+closest_ij['dist'])  # Record the new sum of deltas
...
def main(name):
    ...
    df.to_csv(data_path+'{}_CC_Random_data.csv'.format(name), sep=",", na_rep='', float_format=None, columns=None, header=True, index=True, index_label=None, mode='w', encoding=None,
