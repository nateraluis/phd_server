...
def closest_pair(wcc):
    ...
    closest_pair2 = {'i': 0, 'j': 0, 'dist': np.inf}
    ...
    return closest_pair, closest_pair2
...
def get_data(G_bike, name):
    ...
    for cc in range(to_iterate):
        ...
        # Get the clossest pair of nodes between the two LCC's
        ...
        i_s2.append(closest_ij2['i'])  # Store the sequence of links connected
        ...
        if closest_ij2['i'] != closest_ij2['j']:
            G_bike.add_edge(closest_ij2['i'], closest_ij2['j'], length=0)  # closest_ij['dist'
        ncc += 1
...
def main(name):
    ...
    df.to_csv(data_path+'{}_CC_data_Greedy_Closest.csv'.format(name), sep=",", na_rep='', float_format=None, columns=None, header=True, index=True, index_label=None, mode='w', encoding=None,
