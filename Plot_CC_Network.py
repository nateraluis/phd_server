...
def get_wcc(G_bike):
    wcc = [list(cc.edges()) for cc in list(nx.weakly_connected.weakly_connected_component_subgraphs(G_bike))]
...
def plot_wcc_graph(G, name, wcc, path_plot, filter_wcc=False, n_cc=0):
    ...
    fig, ax = ox.plot_graph(G, node_size=0, edge_color=color_edge, fig_height=10, edge_linewidth=width, show=False, close=False)
...
def main(cities):
    for name in cities:
        ...
        for i in range(1,31):
            ...
            print('  {}/31 done in {} seg. Elapsed time: {} min'.format(i, round(time.time()-time_temp, 2),round((time.time()-st)/60,2 )))
