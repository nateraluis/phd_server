...
def plot_nodes_by_cc(cities, layers, colors, shapes, path_plot, normalize):
    ...
    if normalize == True:
        ylabel = fig.text(-0.01, 0.5, 'Normalized Size', va='center',
                          ha='center', rotation='vertical', fontsize=12)
        fig.savefig(path_plot+'{}_Cities_NodeConnectedComponent_Normalized.png'.format(now.date()),
                    dpi=300, bbox_inches='tight', bbox_extra_artists=[title, xlabel, ylabel])
    else:
        ylabel = fig.text(-0.01, 0.5, 'Size', va='center', ha='center',
                          rotation='vertical', fontsize=12)
        fig.savefig(path_plot+'{}_Cities_NodeConnectedComponent.png'.format(now.date()),
                    dpi=300, bbox_inches='tight', bbox_extra_artists=[title, xlabel, ylabel])
