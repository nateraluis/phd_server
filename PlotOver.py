...
def plot_layer(G, color_edge, gdf, name, layer):
    fig, ax = ox.plot_graph(G, fig_height=30, show=False, close=False, edge_color=color_edge,
                            node_color='white', node_alpha=0, node_size=3, edge_linewidth=edge_w, edge_alpha=0.85)
    plt.close()
    for geometry in gdf['geometry'].tolist():
        if isinstance(geometry, (Polygon, MultiPolygon)):
            ...
            for polygon in geometry:
                patch = PolygonPatch(polygon, fc='none', ec='#c1c0ce',
                                     linewidth=2, alpha=1, zorder=-1)
                ax.add_patch(patch)
    margin = 0.02
    ...
    fig.savefig(path_plot+'{}_{}{}.png'.format(now.date(), name, layer),
