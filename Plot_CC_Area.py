...
def plot_area_by_cc(cities, layers, colors, shapes, path_plot, normalize=True):
    ...
    if normalize == False:
        ylabel = fig.text(-0.01, 0.5, r'$Area\ (m^2)$', va='center',
                          ha='center', rotation='vertical', fontsize=12)
        fig.savefig(path_plot+'{}_Cities_AreaConnectedComponent.png'.format(now.date()),
                    dpi=300, bbox_inches='tight', bbox_extra_artists=[title, xlabel, ylabel])
    else:
        ylabel = fig.text(-0.01, 0.5, r'$Area\ Normalized$', va='center',
                          ha='center', rotation='vertical', fontsize=12)
        fig.savefig(path_plot+'{}_Cities_AreaConnectedComponent_Normalized.png'.format(now.date()),
                    dpi=300, bbox_inches='tight', bbox_extra_artists=[title, xlabel, ylabel])
